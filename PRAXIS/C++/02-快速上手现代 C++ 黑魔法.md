...
- 参数绑定如：`std::bind` 、 `std::function`、`export` 等特性也均
被弃用

# 兼容 C

注：C++ 不是 C 的一个超集！ 尽管两者确实有重叠的地方...    

![cTocc](https://cdn.jsdelivr.net/gh/Noxryn/PicGallery@main/img/cTocc.png)
...
decltype 关键字是为了解决关键字只能对变量进行类型推导的缺陷而出现的  

```cpp
decltype(val1) val2; // val2 类型由 val1 类型推导实现 
```
`std::is_same<T,U>` 用于判断 T 和 U 类型是否相同
...
```cpp
// std::is_integral<Type>::value 判断value是否为 Type 类型(整型)
...
template<typename T>
class SuckType;
typedef SuckType<std::vector<T>, std::string> NewType; // 不合法
template<typename T>
class SuckType;
using  NewType = SuckType<std::vector<T>, std::string> // 合法
```
