...
## 引用其它 Makefile
`include ${Makefile filename}` 将其它 Makefile 包含到文件当前位置   
- 首先在当前目录下寻找
...
>在 makefile 的一开始定义 `.RECIPEPREFIX = >` ，那么后面所有的命令前缀将使用 > ， 而不再是 Tab ，直到 .RECIPEPREFIX 被赋给其他值
...
2. vpath 关键字
   ...
   1. 为符合模式 <pattern>的文件指定搜索目录 `vpath <pattern> <directories>` 
   2. 清除符合模式<pattern> 的文件搜索目录 `vpath <pattern>`
   3. 清除所有设置的文件目录`vpath`
...
注：变量是通过 make 的命令行参数设置的，那么 Makefile 中对这个变量的赋值会被忽略。想在 Makefile 中设置这类参数的值，可以使用"override"指示符。其语法是:
```
...
## 指定目标
有一个make的环境变量叫 MAKECMDGOALS，这个变量中会存放所指定的终极目标的列表，如果在命令行上，没有指定目标，那么，这个变量是空值。
```
sources = foo.c bar.c
